    ...
    public class LimitarPeticionesMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext httpContext, ApplicationDbContext context)
        {
            ...
            if(llaveStringValues.Count == 0)
            {
                ...
                await httpContext.Response.WriteAsync("Debe proveer la llave con la cabecera X-Api-Key");
                return;
            }
            ...
            if(llaveDB.TipoLlave == Entidades.TipoLlave.Gratuita)
            {
                ...
                if (limitarPeticionesDTO.PeticionesPorDiaGratuito <= cantidadPeticionesRealizadasHoy)
                {
                    ...
                    await httpContext.Response.WriteAsync("Excedio peticiones por dia.");
                    return;
                }
        ...
        private bool PeticionSuperaAlgunaDeLasRestricciones(LlaveAPI llaveAPI, HttpContext httpContext)
        {
            ...
            var peticionSuperaLasRestriccionesDominio =
                PeticionSuperaLasRestriccionesDominio(llaveAPI.RestriccionesDominio, httpContext);

            var peticionSuperaLasRestriccionesDeIP =
                PeticionSuperaLasRestriccionesDeIP(llaveAPI.RestriccionesIP, httpContext);

            return peticionSuperaLasRestriccionesDominio || peticionSuperaLasRestriccionesDeIP;
        }
        private bool PeticionSuperaLasRestriccionesDominio(List<RestriccionDominio> restriccions,
            HttpContext httpContext)
        {
            ...
            var superaRestriccion = restriccions.Any(x => x.Dominio == dominio);
